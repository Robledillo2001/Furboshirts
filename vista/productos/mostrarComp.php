    ...
    <main class="contenido-gestion">
        <div class="container-tabla">
            <div class="header-seccion">
                <h2 class="titulo-seccion">Competiciones</h2>
            ...
            <table class="tabla-gestion">
                ...
                <tbody>
                    ...
                    if (!empty($compPag)): ?>
                        <?php foreach ($compPag as $c): ?>
                            <tr>
                                <td><strong><?= $c['ID_COMP'] ?></strong></td>
                                <td><strong><?= htmlspecialchars($c['NOMBRE_COMP']) ?></strong></td>
                                <td><span class="badge"><?= htmlspecialchars($c['TIPO_COMP']) ?></span></td>
                                <td>
                                    <a href="index.php?action=EditarCompeticiones&id=<?=$c['ID_COMP']?>" class="btn-icon edit"><i class="fas fa-edit"></i></a>
                                    <a href="index.php?action=EliminarCompeticiones&id=<?=$c['ID_COMP']?>" class="btn-icon delete" onclick="return confirm('¿Eliminar esta Temporada?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
