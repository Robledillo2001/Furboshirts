    ...
    <main class="contenido-gestion">
        ...
        <div class="container-tabla">
            <div class="header-seccion">
                <h2 class="titulo-seccion">Gestión de Selecciones</h2>
                <div class="botones-header">
                    <a href="index.php?action=AnadirSeleccion" class="btn-anadir">
                        <i class="fas fa-globe"></i> Nueva Seleccion
                    </a>
            ...
            <table class="tabla-gestion">
                ...
                <tbody>
                    <?php if (!empty($seleccionesPag)): ?>
                        <?php foreach ($seleccionesPag as $s): ?>
                            <tr>
                                <td><?= $s['ID_EQUIPO'] ?></td>
                                <td><?= htmlspecialchars($s['NOMBRE_EQUIPO']) ?></td>
                                <td><img src="<?= htmlspecialchars($s['ESCUDO']) ?>" alt=""></td>
                                <td>
                                    <a href="index.php?action=EditarED&id=<?= $s['ID_EQUIPO'] ?>&from=GestionSelecciones" class="btn-icon edit"><i class="fas fa-edit"></i></a>
                                    <a href="index.php?action=EliminarED&id=<?= $s['ID_EQUIPO'] ?>&from=GestionSelecciones" class="btn-icon delete" onclick="return confirm('¿Eliminar esta Seleecion?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
